...
// Girdiden sadece rakamları alır, baştaki 0 veya 90'ı temizler
...
export default function PhoneInput({ label, value, onChange, hint }) {
  ...
  const handleChange = (e) => {
    const yeni = sadeceRakam(e.target.value)
    onChange(yeni)
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-stretch rounded-lg overflow-hidden transition-colors"
        style={{ border: `1px solid ${kenarRengi}` }}>
      ...
      {!bosMu && !gecerli && (
        <span className="text-xs" style={{ color: '#F59E0B' }}>
          Numara 10 haneli olmalı — {rakamlar.length}/10
        </span>
      )}
      {hint && bosMu && (
        <span className="text-xs" style={{ color: textSecondary }}>{hint}</span>
      )}
    </div>
  )
}