...
export const gosterTelefon = (deger, ulkeKodu = '90') => {
  const d = sadeceRakam(deger)
  if (d.length !== 10) return d ? `+${ulkeKodu} ${d}` : '-'
  return `+${ulkeKodu} ${d.replace(/(\d{3})(\d{3})(\d{4})/, '($1) $2 $3')}`
}

const ULKE_KODLARI = [
  { kod: '90', bayrak: '🇹🇷', ad: 'Türkiye' },
  { kod: '1', bayrak: '🇺🇸', ad: 'ABD' },
  { kod: '44', bayrak: '🇬🇧', ad: 'İngiltere' },
  { kod: '49', bayrak: '🇩🇪', ad: 'Almanya' },
]

export default function PhoneInput({ label, value, onChange, hint, ulkeKodu = '90', onUlkeKoduChange }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-stretch rounded-lg overflow-hidden" style={{ border: `1px solid ${kenarRengi}` }}>
        {onUlkeKoduChange ? (
          <select value={ulkeKodu} onChange={(e) => onUlkeKoduChange(e.target.value)}
            className="px-2 text-sm font-semibold outline-none border-r cursor-pointer"
            style={{ backgroundColor: prefixBg, color: textPrimary, borderColor: kenarRengi }}>
            {ULKE_KODLARI.map((u) => (
              <option key={u.kod} value={u.kod}>{u.bayrak} +{u.kod}</option>
            ))}
          </select>
        ) : (
          <span className="flex items-center px-3 text-sm font-semibold select-none border-r"
            style={{ backgroundColor: prefixBg, color: textPrimary, borderColor: kenarRengi }}>+{ulkeKodu}</span>
        )}
        <input
